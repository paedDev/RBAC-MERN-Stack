...
const SideBar = ({ children }) => {
  ...
  return (
    <aside className="min-h-screen">
      <nav
        className={`flex flex-col justify-between  border-r border-gray-200 shadow-xl h-full p-4  ${
          expanded ? "w-20" : "w-64"
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <img
            src="/images/Logo.png"
            alt=""
            className={`${expanded ? "size-8" : "size-10"} `}
          />
        ...
        {/* middle part */}
        <ul className="flex-1 px-1 py-4">{children}</ul>
        ...
        <div className="flex gap-2 items-center relative w-full">
          ...
          <div className="flex items-center gap-2 w-full justify-between">
            ...
            {open && (
              <div
                className={`bottom-10 mt-2  bg-white border-rounded  z-10 text-xs ${
                  expanded
                    ? "absolute -left-1 bottom-15"
                    : " absolute left-[30%]"
                }`}
              >
                <button
...
export const SideBarItem = ({ icon, text, active, alert, to }) => {
  ...
  return (
    ...
    >
      <div
        className={`relative flex items-center group w-full px-3 py-2 rounded-md ${
          expanded ? "justify-center" : "justify-start space-x-2"
        }`}
      >
